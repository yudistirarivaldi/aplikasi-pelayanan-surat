...
<?php
if(isset($_GET['id_request_skm'])){
    $id=$_GET['id_request_skm'];
	$tampil_nik = "SELECT * FROM data_request_skm NATURAL JOIN data_user WHERE id_request_skm=$id";
	$query = mysqli_query($konek,$tampil_nik);
    $data = mysqli_fetch_array($query,MYSQLI_BOTH);
    $id=$data['id_request_skm'];
	$nik = $data['nik'];
    $nama = $data['nama'];
    $ktp = $data['scan_ktp'];
    $kk = $data['scan_kk'];
						...
						<form method="POST" enctype="multipart/form-data">
							<div class="card">
								<div class="card-header">
									<div class="card-title">UBAH REQUEST SKM</div>
								</div>
								<div class="card-body">
									<div class="row">
											...
											<div class="col-md-6 col-lg-6">	
												...
												<div class="form-group">
													...
													<img src="../dataFoto/scan_kk/<?= $kk;?>" value="<?= $kk;?>" width="200" height="100" alt="">
												</div>
                                                <div class="form-group">
													<input type="file" name="kk" class="form-control" size="37">
												</div>
												<div class="form-group">
													<label>Scan Akta</label><br>
													<img src="../dataFoto/scan_akta/<?= $akta;?>" value="<?= $akta;?>" width="200" height="100" alt="">
												</div>
                                                <div class="form-group">
													<input type="file" name="akta" class="form-control" size="37">
												</div>
												<div class="form-group">
...
if(isset($_POST['ubah'])){
	$keperluan = $_POST['keperluan'];
    	...
    	$file_akta = $_POST['nik']."_".".jpg";

		$nama_photo = isset($_FILES['photo']);
    	$file_photo = $_POST['nik']."_".".jpg";
    $sql = "UPDATE data_request_skm SET
    keperluan='$keperluan',
    scan_ktp='$file_ktp',
    scan_kk='$file_kk', 
    scan_akta='$file_akta', 
    scan_foto='$file_photo' WHERE id_request_skm=$id";
	$query = mysqli_query($konek,$sql);

	if($query){
		copy($_FILES['ktp']['tmp_name'],"../dataFoto/scan_ktp/".$file_ktp);
		copy($_FILES['kk']['tmp_name'],"../dataFoto/scan_kk/".$file_kk);
		copy($_FILES['akta']['tmp_name'],"../dataFoto/scan_akta/".$file_akta);
		copy($_FILES['skm']['tmp_name'],"../dataFoto/scan_foto/".$file_photo);
		echo "<script language='javascript'>swal('Selamat...', 'Ubah Berhasil', 'success');</script>" ;
		echo '<meta http-equiv="refresh" content="3; url=?halaman=tampil_status">';
	  }else{
		echo "<script language='javascript'>swal('Gagal...', 'Ubah Gagal', 'error');</script>" ;
		echo '<meta http-equiv="refresh" content="3; url=?halaman=ubah_skm">';
	  }
}
