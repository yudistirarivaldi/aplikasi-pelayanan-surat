								...
								<div class="card-body">
									<div class="row">
											...
											<div class="col-md-6 col-lg-6">		
												...
												<div class="form-group">
													<label>Scan Keterangan Kelahiran</label>
													<input type="file" name="skk" class="form-control" size="37" required>
												</div>
											</div>
									</div>
								</div>
...
if(isset($_POST['kirim'])){
	...
	$keperluan = $_POST['keperluan'];
		...
		$nama_skk = isset($_FILES['skk']['name']);

		$xx = $rand1.'_'.$nama_ktp.'.'.'jpg';
		$ww = $rand4.'_'.$nama_kk.'.'.'jpg';
		$yy = $rand4.'_'.$nama_skk.'.'.'jpg';
		$sql = "INSERT INTO data_request_skk (nik,scan_ktp,scan_kk,scan_skk, keperluan) VALUES ('$nik','$xx','$ww','$yy','$keperluan')";
		$query = mysqli_query($konek,$sql) or die (mysqli_error());

	if($query){
		copy($_FILES['ktp']['tmp_name'],"../dataFoto/scan_ktp/".$xx);
		copy($_FILES['kk']['tmp_name'],"../dataFoto/scan_kk/".$ww);
		copy($_FILES['skk']['tmp_name'],"../dataFoto/scan_skk/".$yy);
