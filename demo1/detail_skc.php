...
<?php
	if(isset($_GET['id_request_skc'])){
		$id_request_skc=$_GET['id_request_skc'];
		$sql = "SELECT * FROM data_request_skc natural join data_user WHERE id_request_skc='$id_request_skc'";
								...
								<div class="card">
									<div class="card-header">
										...
										<label>Keterangan</label>
											...
											<a href="?halaman=sudah_acc_skc" class="btn btn-info btn-border btn-round btn-sm">
												Batal
											</a>
											</div>
									</div>
...
<?php
	if(isset($_POST['ubah'])){
		...
		if($query==1){
			...
			$ubah = "UPDATE data_request_skc SET
		keterangan='$keterangan' WHERE id_request_skc='$id_request_skc'";
		$quey = mysqli_query($konek,$ubah);
		if($quey==1){
			echo "<script language='javascript'>swal('Selamat...', 'Ubah Berhasil', 'success');</script>" ;
			echo '<meta http-equiv="refresh" content="3; url=?halaman=sudah_acc_skc">';

		}
		  }else{
			echo "<script language='javascript'>swal('Gagal...', 'Ubah Gagal', 'error');</script>" ;
			echo '<meta http-equiv="refresh" content="3; url=?halaman=detail_skc">';
		  }
	}
?>